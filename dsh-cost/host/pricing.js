/**
 * Price table for the cost pill: model rates in a currency per one million
...
function rateField(value, model, tier, key) {
  ...
  if (typeof raw !== 'number' || !Number.isFinite(raw) || raw < 0) {
    throw new Error(`dsh-cost: pricing.${model}.${tier}.${key} must be a non-negative number`)
  }
  return raw
}
...
function normalizeRate(value, model, tier) {
  if (value === null || typeof value !== 'object') {
    throw new Error(`dsh-cost: pricing.${model}.${tier} must be an object with ${RATE_KEYS.join('/')}`)
  }
  const rate = {}
...
function normalizeModel(value, model) {
  if (value === null || typeof value !== 'object') {
    throw new Error(`dsh-cost: pricing.${model} must be an object or null to drop the built-in rate`)
  }
...
export function normalizePricing(pricing) {
  ...
  if (pricing !== undefined) {
    if (pricing === null || typeof pricing !== 'object' || Array.isArray(pricing)) {
      throw new Error('dsh-cost: config.pricing must be an object mapping model ids to rates')
    }
 ...
 * @param rows - folded rows from `foldUsage`.
