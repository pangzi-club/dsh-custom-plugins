 ...
 * envelope the client module table expects, expose the plugin exports, and
 * register the dock entry with a locale namespace that has zh/en parity.
 */
...
test('presents the loader envelope with the package identity', () => {
  ...
  assert.equal(row.id, 'dsh-cost')
  assert.equal(typeof row.factory, 'function')
  assert.equal(exports.name, 'dsh-cost')
...
test('injects its stylesheet once and registers the dock entry', () => {
  ...
  exports.apply(ctx)
  const style = created.find(element => element.dataset.plugin === 'dsh-cost')
  assert.ok(style, 'a plugin-scoped style element is created')
  assert.match(style.textContent, /dsh-cost-pill/)
  assert.equal(registered.locale.ns, 'dsh-cost')
  assert.equal(registered.inject, 'conversation.composer.dock')
...
test('ships zh/en dictionaries with identical key sets', () => {
  ...
  assert.ok(Object.keys(plain(zh)).length >= 20)
})
